...
function validateDefaults(raw: unknown, errors: ConfigError[]): CatalogDefaults | undefined {
  ...
  const {
    substrateId, grammage, sheetSizeId, pageWidth_mm, proportionId, bleed_mm, totalPages,
  } = raw;
  let valid = true;
  ...
  if (!valid || !isCleanIdentifier(substrateId) || !isFiniteNumber(grammage)
    ...
    || !isPositiveSafeInteger(totalPages)) {
    return undefined;
  }

  return { substrateId, grammage, sheetSizeId, pageWidth_mm, proportionId, bleed_mm, totalPages };
}
...
/**
 * Validate the three parsed catalog files together and collect every error found,
