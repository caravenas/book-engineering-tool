...
export function CoverResults() {
  ...
  if (plan && plan.kind === 'blanda') {
    return (
      <>
        <ResultList>
          <ResultRow label="Ancho del pliego de tapa (mm)">{formatMm(plan.sheetWidth_mm)}</ResultRow>
          <ResultRow label="Alto del pliego de tapa (mm)">{formatMm(plan.sheetHeight_mm)}</ResultRow>
          <ResultRow label="Peso del papel de tapa">{formatWeight(plan.paperWeight_g)}</ResultRow>
        </ResultList>
  ...
  if (plan && plan.kind === 'dura') {
    return (
      <>
        <ResultList>
          <ResultRow label="Ancho del cartón lateral (mm)">{formatMm(plan.boardWidth_mm)}</ResultRow>
          <ResultRow label="Alto del cartón (mm)">{formatMm(plan.boardHeight_mm)}</ResultRow>
          <ResultRow label="Ancho del cartón de lomo (mm)">{formatMm(plan.spineBoardWidth_mm)}</ResultRow>
          <ResultRow label="Ancho del forro (mm)">{formatMm(plan.wrapWidth_mm)}</ResultRow>
          <ResultRow label="Alto del forro (mm)">{formatMm(plan.wrapHeight_mm)}</ResultRow>
          <ResultRow label="Peso del forro">{formatWeight(plan.paperWeight_g)}</ResultRow>
          <ResultRow label="Área de cartón lateral (m²)">{formatArea(plan.sideBoardArea_m2)}</ResultRow>
          <ResultRow label="Área de cartón de lomo (m²)">{formatArea(plan.spineBoardArea_m2)}</ResultRow>
          <ResultRow label="Área total de cartón (m²)">{formatArea(plan.boardArea_m2)}</ResultRow>
        </ResultList>
        <p className="calculation-note" style={{ marginTop: 'var(--space-2)' }}>
